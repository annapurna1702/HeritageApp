    ...
    if lat and lon:
        ...
        for place in places:
            ...
            if distance < 10000:  # Within 10 kilometers
                st.success(f"You are **in** {place['name']}! Details: {place['details']}")
                in_place = True
                nearest_place = place
                break
            elif distance < 100000:  # Within 100 kilometers
                st.success(f"You are **near** {place['name']}. Details: {place['details']}")
                in_place = True
                nearest_place = place
        ...
        if username in expert_credentials and expert_credentials[username] == password:
            ...
            if user_appointments:
                for appointment in user_appointments:
                    st.write(f"**{appointment['name']}** at {appointment['time']}")
            else:
